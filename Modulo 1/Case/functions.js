...
newJson.forEach((obj) => {
  let quantity = obj.item.quantity; // as variaveis criadas aqui podem ser um const, já que não se altera
...
// as funções que só retornam valor pode ser um arrowFunction, mas não é obrigatório
...
// usar operação ternária encurta a função
// const firstChangeFor = (methodPath, value) => methodPath === 'CASH' ? Number((value + value * 0.07).toFixed(4)) : 0;
// outro ponto é, validar primeiro o erro e depois o sucesso faz com que vc não precise do else
// exemplo:
function firstChangeFor2(methodPath, value) {
  if (methodPath !== "CASH") return 0;
